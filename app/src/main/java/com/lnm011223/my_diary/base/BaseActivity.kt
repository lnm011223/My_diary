...
abstract class BaseActivity : AppCompatActivity(){
    ...
    public fun circularReveal(intent: Intent?){
    ...
    public fun circularRevealReverse(intent: Intent?){
        //系统提供的揭露动画需 5.0 及以上的 sdk 版本，当我们获取不到上个界面的点击区域时就不展示揭露动画，因为此时没有合适的锚点
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP ||
        ...
        mAnimRevealR?.start().apply {
            Log.d("反揭露",mAnimRevealR.toString())
        }
        Log.d("反揭露",localClassName)
